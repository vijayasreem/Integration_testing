...
public interface LoanApplicationRepository extends JpaRepository<LoanApplication, Long> {
    ...
    // Query to retrieve loan applications by customer ID and approval status
    @Query("SELECT la FROM LoanApplication la WHERE la.customerId = :customerId AND la.approvalStatus = :approvalStatus")
    List<LoanApplication> findByCustomerIdAndApprovalStatus(Long customerId, String approvalStatus);

    // Query to retrieve loan applications by customer ID and submission date range
    @Query("SELECT la FROM LoanApplication la WHERE la.customerId = :customerId AND la.submissionDate >= :startDate AND la.submissionDate <= :endDate")
    List<LoanApplication> findByCustomerIdAndSubmissionDateRange(Long customerId, LocalDate startDate, LocalDate endDate);
}
